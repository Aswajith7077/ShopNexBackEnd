...
CREATE PROCEDURE CHECK_USER_EXISTS(IN EMAIL_ID VARCHAR(100),IN PASSWRD VARCHAR(50))
BEGIN
	IF (EMAIL_ID IN (
			SELECT EMAIL
			FROM USER_LOGIN
        	...
        	SELECT PASSWD
            FROM USER_LOGIN
            WHERE EMAIL_ID = USER_LOGIN.EMAIL AND PASSWRD = PASSWD
        )
	)
	THEN
		SELECT 'SUCCESS';
...
CREATE PROCEDURE LOGIN_USER(IN EMAIL_ID VARCHAR(100),IN PASSWRD VARCHAR(50))
BEGIN
    IF (
        PASSWRD IN (
            SELECT PASSWD
...
CREATE PROCEDURE CREATE_USER(IN EMAIL_ID VARCHAR(100),IN PASSWD)
BEGIN
    IF (
        EMAIL_ID NOT IN (
            SELECT EMAIL
    ...
    THEN
        INSERT INTO USER_LOGIN (EMAIL,PASSWD,POINTS,IS_LOGGED)
        VALUES(EMAIL_ID,PASSWRD,0,FALSE);
        SELECT 'SUCCESS';
    ELSE
        SELECT 'FAILURE';
    END IF;
...
CREATE PROCEDURE SIGN_OUT(IN EMAIL_ID VARCHAR(100))
BEGIN
    IF (EMAIL_ID IN (
        SELECT EMAIL
        FROM USER_LOGIN
...
CREATE PROCEDURE CHANGE_PASSWORD(IN EMAIL_ID VARCHAR(100),IN NEW_PASSWD VARCHAR(50))
BEGIN

...
from services.auth import Authenticate,CurrentUser
...
async def check_refresh_token(refresh_token:str):
    ...
    if result is None:
        raise HTTPException(status_code = status.HTTP_406_NOT_ACCEPTABLE, detail = "Invalid Refresh Token")
...
async def login_user(form_data:UserLogin):
    ...
    if result is None:
        raise HTTPException(status_code = status.HTTP_401_UNAUTHORIZED,detail = "Either User not present or Invalid password")
    ...
    return {"username":form_data.username,"fullname":user_data['fullname'],"access_token":access_token,"refresh_token":refresh_token}
